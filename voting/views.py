...
def dashboard(request):
    ...
    if user.voter.otp is None or user.voter.verified == False:
        ...
        if user.voter.voted:  # * User has voted
            ...
            blockchain_data = []
            blockchain_blocks = DbBlock.objects.all()
            ...
            for block in blockchain_blocks:
                ...
                for transaction in transactions:
                    voter_info = Voter.objects.get(pk=transaction.voter)
                    candidate_info = Candidate.objects.get(pk=transaction.candidate)
                    position_info = Position.objects.get(pk=candidate_info.position_id)

                    transaction_data = {
                        'voter_id': transaction.voter,
                        'voter_name': voter_info.admin.get_full_name(),
                        'voter_phone': voter_info.phone,
                        'candidate_id': transaction.candidate,
                        'candidate_name': candidate_info.fullname,
                        'candidate_photo': candidate_info.photo.url,
                        'candidate_bio': candidate_info.bio,
...
from .blockchain import Block, Blockchain, TransactionModel
import datetime
import candidate_checker
...
def submit_ballot(request):
    ...
    if(candidate_checker.check_is_voter_valid(str(request.user))==False):
        messages.error(request, "The provided face does not match the registered voter information. Please try again.")
        return redirect(reverse('show_ballot'))
            ...
            if len(form_position) > max_vote:
                ...
                for form_candidate_id in form_position:
                    ...
                    try:
                        ...
                        transactions.append({"voter": voter.id, "candidate": candidate.id, "position": position.id})
    ...
    new_block = Block(len(blockchain.chain), blockchain.chain[-1].hash, datetime.datetime.now(), transactions)
...
def get_voting_results(request):
    blockchain_data = []
    blockchain_blocks = DbBlock.objects.all()
    ...
    return JsonResponse({'blockchain': blockchain_data})