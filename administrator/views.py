...
from voting.models import Voter, Position, Candidate, Votes, VoterPhoto
...
def upload_voter_photo(request):
    if request.method == 'POST' and request.FILES.get('photo'):
        try:
            # Get the current voter being created/edited
            voter_id = request.session.get('current_voter_id')
            voter = Voter.objects.get(id=voter_id)
            
            # Save the photo
            photo = request.FILES['photo']
            VoterPhoto.objects.create(
                voter=voter,
                photo=photo
            )
...
from django.http import JsonResponse
from voting.models import Block as DbBlock
from voting.models import Transaction

def getVotingResults(request):
    ...
    return render(request, 'voting_results.html', {'blockchain': blockchain_data})